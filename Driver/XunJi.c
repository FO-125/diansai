...
void XunJIPIDConcrol(uint16_t EncoderNumber)
{
    ...
    // 添加 a flag to control deceleration only once
    uint8_t decelerated = 0;  // 0: not decelerated, 1: decelerated
    uint16_t originalCentrePWM = CentrePWM;  // save original speed
    ...
    while(1)
    {
        ...
        // Only decelerate once when reaching 80% of target distance
        // if(Average_count >= EncoderNumber * 0.5 && decelerated == 0)
        // {
        //     CentrePWM *= 0.5;
        //     decelerated = 1;  // set flag to prevent re-execution
        // }
...
//       if(Average_count>=EncoderNumber*0.8)
//         CentrePWM*=0.5;
